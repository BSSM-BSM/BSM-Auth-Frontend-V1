...
const Oauth = ({
  ...
  useEffect(() => {
    if (clientId && redirectURI) authenticate();
  ...
  return (
    <div>
      ...
      {user.isLogin && (
        ...
        >
          <div className="cols gap-1">
            ...
            <Button className="accent" full onClick={() => authorize()}>
              인증
            </Button>
          </div>
        </Modal>
      ...
      <Modal id="oauthAuthenticateFailed" title="OAuth 인증에 실패하였습니다">
        ...
        <br />
        <Button className="accent" full onClick={() => authenticate()}>
          다시 시도
        </Button>
      </Modal>
      <Modal id="oauthAuthorizeFailed" title="OAuth 인가에 실패하였습니다">
        <p>
          클라이언트 서버를 인증할 수 없거나 서버 내부에서 문제가 발생하였습니다
        </p>
        <p>잠시후에 다시 시도해주세요.</p>
        <br />
        <Button className="accent" full onClick={() => authorize()}>
